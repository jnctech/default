# Image-gen prompt — homelab network topology (NEON variant)

> One of three aesthetic-specific variants. Source of truth:
> `diagrams/homelab-network-extended.svg`.
>
> **Direction:** neon-on-true-black cyberdeck panel.
...
below, rendered as a **neon-on-black cyberdeck panel**. Canvas
~1080:4250, designed for a tall Samsung Galaxy AMOLED phone.

### Aesthetic — neon cyberdeck
Pure #000 AMOLED background. Vivid saturated neons glowing against the
black — pick your own coherent neon palette (cyan, magenta, lime, hot
pink, electric violet, sodium amber). Each tier reads as a distinct
neon zone with a faint outer bloom, like illuminated signage seen at
night. Connecting lines feel like fibre-optic strands or laser tracks —
slight bloom, soft falloff. Subtle CRT scan-lines or faint chromatic
aberration on titles is welcome but not mandatory. Borders can be
angular and slightly brutalist. Typography: terminal-style monospace
for everything technical (IPs, ports, CIDRs) — possibly with a sci-fi
geometric / wide stencil for tier titles. Glyphs are simple geometric
primitives rendered as glowing line-art, not photo-real icons. Mood:
1990s network operations center in a film about hackers — slightly
grimy, beautifully high-contrast, the kind of panel that would look
good photographed in a dark room.
...
3. **Two remote-access paths** from Internet:
   ...
   - Alternative **WireGuard** node, subtitle *"Remote Access · alt
     ...
     bottom — render it as a long arcing neon trace on one side.
...
10. **IoT Network · VLAN 70 — ⚠ PLANNED, NOT DEPLOYED.** Render this
    zone with its neon **off / flickering / unlit** — a "vacancy" sign
    that's been turned off. Dashed outline only, body desaturated to
    near-grey, with the warning badge in a bright amber/sodium glow so
    it remains readable. Clearly not a live zone.
...
11. **VLAN Inventory · RB4011 — 8 VLANs**, each chip glowing in its own
    accent neon:
...
12. **Network Equipment · MikroTik** — four devices, each a glowing
...
13. **Security · Open Issues** — three warning cards, each glowing hot
    red / blood neon:
...
14. **Footer** in tiny dim grey: *"Portrait 1080×4250 · True-black OLED ·
...
- VLAN 70 / IoT rendered as unlit / flickering — clearly not live.
- Security cards in hot red neon — clearly warnings.
- Background must be true #000 black, not dark grey.
