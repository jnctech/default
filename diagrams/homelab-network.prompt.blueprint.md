# Image-gen prompt — homelab network topology (BLUEPRINT variant)

> One of three aesthetic-specific variants. Pair it with the same SVG source
> of truth: `diagrams/homelab-network-extended.svg`. The facts payload is
> identical across variants — only the aesthetic prescription changes.
>
> **Direction:** engineering blueprint / CAD plate.
...
below, rendered as an **engineering blueprint**. Canvas ~1080:4250,
designed to be scrolled top-to-bottom on a tall Samsung Galaxy AMOLED
phone.

### Aesthetic — blueprint / drafting plate
Treat this as a CAD plate or printed-circuit layout. Background is a
deep cyanotype field (deep teal or Prussian blue) with the diagram drawn
in pale-blue and white hairlines. Use **technical drafting language**
throughout: hairline rules at consistent weights, dimensional callouts
with arrow leaders, register marks and crop marks at the corners, a
small title block in one corner with revision metadata, faint grid
ruling behind the content. Zones look like labelled sections of a
schematic; node glyphs look like component symbols on a diagram
(resistors, IC packages, plate symbols — your interpretation), not
photo-real icons. Typography: a CAD-style geometric (think AutoCAD's
RomanS or a clean drafting sans) with a precise monospace for IPs,
ports, and dimensions. Restrained palette: paper-cyan field, white and
pale-blue lines, a single warm accent (amber/copper) reserved for
PLANNED / warning callouts, and a deep oxide-red reserved for the
security flags. Mood: drafting table at 2 a.m., precise, unhurried,
slightly worn. The poster should look like it could be tacked to a
machinist's wall.
...
3. **Two remote-access paths** from Internet:
   ...
   - Alternative **WireGuard** node, subtitle *"Remote Access · alt
     ...
     Cloudflare entirely and curves down to the RB4011 router in the
     equipment band — render as a long arcing dimensional leader.
...
8. **Inter-network routing** arrow with a **Firewall model** callout:
...
10. **IoT Network · VLAN 70 — ⚠ PLANNED, NOT DEPLOYED.** Treat as a
    section drawn in faded construction lines / phantom lines per
    drafting convention — clearly not part of the as-built diagram.
...
11. **VLAN Inventory · RB4011 — 8 VLANs**:
...
12. **Network Equipment · MikroTik** — four devices, rendered as drafted
    component symbols:
...
13. **Security · Open Issues** — three warning callouts in oxide-red:
...
14. **Footer** in the title block: *"Portrait 1080×4250 · True-black OLED
...
- VLAN 70 / IoT in phantom / construction lines — clearly not as-built.
- Security cards in oxide-red as warning callouts.
